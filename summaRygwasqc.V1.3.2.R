...
option_list <- list(
  make_option(c("-a","--GWAS"), action="store_true", type="character", help="Location of GWAS Summary Statistics"),
  make_option(c("-b","--HRC"), action="store_true", type="character", help="Location and prefix of HRC reference panel information"),
  make_option(c("-c","--chr"), action="store_true", type="character", help="CHR column name"),
  make_option(c("-d","--bp"), action="store_true", type="character", help="BP column name"),
  make_option(c("-e","--effectallele"), action="store_true",type="character", help="Column name for allele corresponding to or/beta/z"),
  make_option(c("-f","--nonEA"), action="store_true",type="character", help="Column name for allele NOT corresponding to or/beta/z"),
  make_option(c("-g","--beta"), action="store_true", type="character", help="BETA column name"),
  make_option(c("-v","--or"), action="store_true", type="character", help="OR column name"),
  make_option(c("-i","--z"), action="store_true",type="character", help="Z-Score column name"),
  make_option(c("-j","--pvalue"), action="store_true",type="character", help="Column name for GWAS p-value"),
  make_option(c("-k","--se"), action="store_true",type="character", help="Column name for standard error (e.g SE)"),
  make_option(c("-l","--info"), action="store_true",type="character", help="Column name for info or imputation quality (e.g INFO)"),
  make_option(c("-m","--snp"), action="store_true",type="character", help="Column name for SNP or rsID (e.g SNP"),
  make_option(c("-n","--frq"), action="store_true",type="character", help="Column name for Allele 1 frequency (e.g FRQ)"),
  make_option(c("-o","--PRS"), action="store_true",type="character", help="Derive polygenic risk scores?"),
  make_option(c("-p","--prsice2"), action="store_true",type="character", help="Full path to PRSice2 binary"),
  make_option(c("-q","--target"), action="store_true",type="character", help="Full path and prefix for target bed/bim/fam"),
  make_option(c("-r","--plink1.9"), action="store_true",type="character", help="Full path to plink1.9"),
  make_option(c("-s","--prsice2R"), action="store_true",type="character", help="Full path and prefix for PRSice.R"),
  make_option(c("-x","--ithres"), action="store_true",type="character", help="INFO score threshold"),
  make_option(c("-u","--mthres"), action="store_true",type="character", help="MAF score threshold"),
  make_option(c("-y","--MHC"), action="store_true",type="character", help="Remove MHC"),
  make_option(c("-z","--N"), action="store_true",type="character", help="Column name for total sample size")
)
...
logmestart("Running summaRygwasqc v1.3.2")
cat(paste("Running summaRygwasqc v1.3.2\n"))
...
} else if(length(N.matches)==0){
  warning("GWAS does not have a N Samples column")
}
...
info.labels=c("INFO","info","R2","r2","INFO-score","Info-score","info-score")
...
se.labels=c("SE","se","standarderror","standard_error","Standard_error","Standard_Error","STANDARD_ERROR","STANDARDERROR","StdErr","StdErrLogOR","SEBETA")
...
### Pvalue class
if(class(GWAS[,p.val.col])=="character"){
...
### Info class
if(length(info.matches)>0){
  if(class(GWAS[,info.col])=="character"){
...
### Effect size class
if((length(beta.matches)>0 | length(or.matches)>0 | length(z.matches)>0) & class(GWAS[,effect.col])=="character"){
...
write.table(all.chrs.cleaned,gz.output,col.names=T,row.names=F,quote=F)
...
write.table(all.chrs.cleaned.overlap,gz.output,col.names=T,row.names=F,quote=F)
