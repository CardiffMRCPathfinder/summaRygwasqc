...
logmestart("Running summaRygwasqc v1.3.3")
cat(paste("Running summaRygwasqc v1.3.3\n"))
...
if(length(N.matches)>0){
  ...
  N.text=paste("GWAS does not have a N Samples column",sep="")
...
if(prscs==T){
  ...
  write.table(all.chrs.cleaned[,c(1,4,5,7)],prscs.output,col.names=T,row.names=F,quote=F,sep="\t")
}
